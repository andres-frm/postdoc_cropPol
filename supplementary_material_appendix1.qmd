...
subtitle: 'Appendix S1. Journal: Ecological Modelling'
...
We conducted this study in the Yungas ecoregion, a subtropical Andean forest that, in northwestern Argentina, ranges from 22° to 29° S. This region has a marked seasonality with low rainfall between June and October and high rainfall between November and March. The study sites were located roughly at 400 m a.s.l. at the Yungas piedmont in Tucumán province, whose original wooded vegetation was almost completely replaced by agricultural expansion. Currently, the agricultural landscape of Yungas piedmont is a mosaic of scattered patches of natural cover and pollination-dependent crops, such as blueberry and strawberry, embedded in a large-scale matrix of monoculture systems of sugar cane, citrus, and soybean.
...
We collected the data in the province of Entre Ríos, Argentina. Climatic characteristics of the region include hot and humid summers, mild winters, rainfall distributed throughout the year, high relative humidity, prevailing winds from the north and northeast, and the presence of fog during the colder months.

Along two seasons (2016 and 2021), we did pollinator exclusion experiments to quantify the effect of animal pollination in blueberry fruit set. To do so, we sampled nine farms (~1 ha) of the Emerald cultivar with an average distance of ~5 km between contiguous farms. The pollination treatments consisted on 5-6 randomly chosen plants per farm: 3 branches were used as open treatments to assess the effect of  pollinators on fruit set, whereas the rest were excluded using white Voile fabric bags to isolate the flowers from floral visitors. Regardless of the treatment, the number of floral buds was counted at the beginning of each branch, removing those in doubt of viability. Then during the harvesting season,the number of developed fruits was counted to estimate the proportion of flowers setting fruit. 
...
fruit_plant$plant_id <- as.factor(paste(fruit_plant$farm, 
                                        fruit_plant$plant, sep = '_'))

fruit_plant$plot <- as.factor(paste(fruit_plant$farm, 
                                    fruit_plant$plot, sep = '_'))

fruit_plant <- fruit_plant[, c("farm", "plot", 'plant_id', "total_fruts")]

fruit_plant <- lapply(fruit_plant, 
...
cat(file = 'fruits_plant.stan', 
    ...
    generated quantities{
      ...
      for (i in 1:N) {
        log_lik[i] = neg_binomial_2_lpmf(total_fruts[i] | mu_1[i], scale);
      }
...
```{r}
ppcheck_tot_fru <- mod_fruit_plant$draws(variables = 'ppcheck', 
                                         format = 'matrix')
...
```{r}
hives_ha <- 
  function(n_hives = 25, # n hives to be simulated
           mu_pop = 1e4, # mu parameter NegBin distribution (i.e. Bees hive-1)
           sigma_pop = 10, # dispersion parameter NegBin distribution. Bees hive-1
  ...
  set.seed(seed) 
  # Simulating the number of bees per hive j 
...
plot(density((foraging_time_HQ/60)/60), col = 'tan1', main = '',
     ylim = c(0, 1.1),
...
par(mfrow = c(3, 3), mar = c(4, 4, 1, 1))
for (i in 1:9) trace_plot(mod_visit_honeybee, 
                          output_mod_visit_honeybee$variable[i], 3)
par(mfrow = c(1, 1))
```
...
plot(density(visits_day), main = '', 
     xlab = 'Number of floral visits of\n a single forager honeybee bee per day',
...
cat(file = 'model_apis_SVP.stan', 
    ...
    generated quantities{
      ...
      for (i in 1:N) {
        log_lik[i] = neg_binomial_2_lpmf(pollen[i] | lambda1[i], phi);
      }
...
set.seed(1234)
sim_visits <- 
  apply(pp_check_svp[c(sample(1:nrow(pp_check_svp), 
                              size = 100, replace = F)),], 1, 
        simplify = 'list', FUN = 
...
cat(file = 'slope_svp_apis.stan', 
    ...
    generated quantities{
      ...
      for (i in 1:N) {
        log_lik[i] = neg_binomial_2_lpmf(poll_ac[i] | lambda1[i], sigma);
      }
...
post_svp_slope <- 
  mod_svp_slope$draws(variables = c('alpha', 'beta'), format = 'df')
...
pollen_surpass <- 
  read_xlsx('/Users/andres/Documents/github_repos/cap4_phd/all_data.xlsx', 
            sheet = 7, na = 'NA')[, -2]

dat_asymptote_pollen <- 
  as_tibble(pollen_fruit[pollen_fruit$tratamiento == 'l', ])

dat_asymptote_pollen$farm <- 'sta_lu'
dat_asymptote_pollen$plant_id <- 
  dat_asymptote_pollen %$% paste(farm, planta, sep = '')
...
cat(file = 'asymptote_pollen1.stan', 
    ...
    generated quantities{
      ...
      for (i in 1:N) {
        log_lik[i] = neg_binomial_2_lpmf(no_polen[i] | mu1[i], scale);
      }
...
abline(v = c(225, 250, 275), 
       lty = c(2, 3, 2), col = 'red') # Aproximate level of saturation
...
```{r}
pollen_deposition_fun <- 
  function(x, # number of flower visits to the flower
...
fruit_size_cultivars[] <- 
  lapply(fruit_size_cultivars, 
         function(x) if(!is.numeric(x)) as.factor(x) else(x))
...
```{r}
predict_fruit_size <- 
  function(x, # stigmatic pollen load
...
simulated_visits <- 
  function(p_ha, # N plants per ha
    ...
    set.seed(seed)
    flowering_perc <- 
      rbeta(p_ha, beta1, beta2) # flowering percentage of the plants
    ...
    if (hive_aggregate) {
      ...
      flowers <- 
        ...
        lapply(foragers_visits, FUN = 
                 function(hive) {
                   for (day in 1:2) { 
                     # number of pollination days (blueberry days of receptivity)
                     
                     message(paste('Starting day', day, 'of pollination'))
                     
                     for (trips_bee in seq_along(hive)) { 
                       # trips of bee i from hive j
      ...
      plants <- 
        lapply(flowers, FUN = 
                 # iteration across flowers visited by honeybees from hive i
                 function(visits_hive_i) {
                   
                   lapply(plants, FUN = 
                            function(plant_i) {
                              
                              sample(visits_hive_i, length(plant_i)) 
                              # assign flower visits to each plant
      ...
      while (i < length(plants)) { 
        # The visits accumulates across each flower from plant i
        ...
        lapply(foragers_visits, FUN = 
                 function(hive) {
                   for (day in 1:2) { 
                     ...
                     for (trips_bee in seq_along(hive)) { 
                       # trips of bee i in hive j
                       # visited flowers within the crop
                       flower_id <- sample.int(sum_flowers, hive[trips_bee]) 
                       # visited flowers
                       # recording the floral visits
                       for (k in seq_along(flower_id)) 
                         flowers[flower_id[k]] <- flowers[flower_id[k]] + 1
      ...
      plants <- 
        lapply(flowers, FUN = 
                 # iteration across flowers visited by bees from hive i
                 function(visits_hive_i) {
...
sim_t_ha <- 
  ...
  geom_label(data = lab, aes(x, y), 
             label = lab$lab, fill = c('tan1', 'lightblue'), size = 3, 
             alpha = 0.7) +
...
knee_plot1 <- 
  ...
  geom_label(data = lab, aes(x, y), label = lab$label, size = 2.5, 
...
contrast_t <- 
  ...
  geom_label(data = lab, aes(x, y), label = lab$lab, size = 2.5,
             label.padding = unit(0.15, 'lines')) +
  theme_bw() +
