...
We also use data from pollen deposition experiments in Snowchaser cultivar in one of the sampled farm (see section 5.1 below), and included stigmas from free pollination treatment for the analysis.
...
\end{align}
$$
We used $\tau$ as the intercept, and $\mu_\tau \sim normal(5, 0.5)$ distribution since encompass the expected average value of pollen deposition. The $normal(0, 1)$ and $exponential(1)$ are standard priors for group level effects and disperssion parameters. We ffited the model using a non-centered parametrization and partial pooling among clusters (i.e. plants, farms and cultivars). 
...
We used posterior predictive distributions of stigmatic pollen deposition for each cultivar, to estimate cumulative distribution functions and approximate the saturation point.
...
Finally, the function `pollen_deposition_fun` simulate pollen deposition in blueberry flowers after N honeybee visits
...
The first part involves a model of fruit diameter of three cultivars to estimate the contrast among them (i.e. size differences). It was necessary since the the following fruit weight estimation from fruit diameter (see section ... ) was conducted using Emerald's data.
```{r}
...
\end{align}
$$
We use $\alpha$ as the model's intercept and $\mu_\alpha \sim normal(15, 2.5)$ prior since encompass the expected diameter of a blueberry fruit. We used a non-centered parametrization and partial pooling to estimate the parameters.
...
We used posterior predictive distributions of fruit size for all cultivars to estimate the contrast (here we included other cultivars, but only emerald and snowchaser are used in the following analyses).
...
Finally, the function `predict_fruit_size` use the posterior distribution of the previous model to simulate expected fruit size given stigmatic pollen deposition.
```{r}
predict_fruit_size <- function(x, # stigmatic pollen load
                               cultivar = 'sch', # abreviation of the cultivar.
...
During the production seasons of 2020 and 2021, we sampled the size of fruits produced nine blueberry farms of the Northwestern Argentina. To do so, we selected one plot per farm twelve plants/plot. We randomly selected ten mature fruits per sampled plant to measure their equatorial diameter with a 0.05 mm accuracy caliper, and fruit weight with a digital balance of 0.01 g resolution. Using this data we fitted a model to estimate fruit weight as function of the diameter.
...
\end{align}
$$
We defined the $\alpha$ parameter as the model's intercept and $normal(1.5, 0.5)$ as prior since encompass the typical weight of of a blueberry fruit. We used using a non-centered parametrization and partial pooling to estimate the parameters.
...
The above 5 steps had the purpose to parametrize the entire simulation. The following sections show the main functions conducting the actual job: honeybees visiting flowers (function `simulated_visits`), pollen deposition at the crop level (function `crop_pollination`), and crop yield (function `crop_yield`).

## Floral visits

The function `simulated_visits` generates the blueberry hectare and recreates the first phase of the pollination: N hives of quality k per hectare, with N foragers each. Each of the individual honeybees conduct a random number of floral visits daily, depending on the quality of its hive. That is, the function simulate the random process of honeybees visiting flowers in the crop, depending on the density and quality of the hives within the bluberry hectare.
```{r}
simulated_visits <- 
...
The function `crop_pollination` simulate pollen deposition in the crop flowers from the visits generated with `simulated_visits`.
...
crop_pollination <- function(p_ha, # plants per ha
  ...
  # the following code conduct data formting tasks
  if (short == F) {
...
The `crop_yield` function use the simulated pollen deposition from `crop_pollination` to estimate the total fruits produced by each plant, their weight, and the yield per ha.
